...
def payload(tripletex_client, fee_dict):
    ...
    return {"date": desc_date, "description": desc_text, "postings": postings_list}
...
def main():
    ...
    zettle_client = Zettle(zettle_config.zettle_id, zettle_config.zettle_secret)
    fee_dict = zettle_client.get_fees(date1,date2)

    tripletex_config = Tripletex_Config()
    tripletex_client = Tripletex(tripletex_config.base_url, tripletex_config.consumer_token, tripletex_config.employee_token, tripletex_config.expiration_date)
